	...
	var body: some View {
		ZStack(alignment: .leading) {
			VStack(alignment: .leading, spacing: 4) {
				HStack(spacing: 8) {
					Text(transaction.partner).font(.system(size: 14, weight: .regular))
					Spacer(minLength: 0)
				}
				if let description = transaction.description {
					Text(description).font(.system(size: 12, weight: .thin))
				}
				...
				HStack(spacing: 0) {
					...
					Text(transaction.date).font(.system(size: 12, weight: .thin))
				}
			}
			.padding(16)
			.background {
